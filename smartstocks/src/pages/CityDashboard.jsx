...
export default function CityDashboard() {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        // const res = await axios.get(
        //   "http://127.0.0.1:8000/api/sales/cities/summary",
        //   {
        //     headers: {
        ...
        //For Deployment
        const res = await api.get("/api/sales/cities/summary");
  ...
  if (loading) {
    return <div className="p-6">Loading City Dashboard...</div>;
  }
  ...
  /* ================= KPI CALCULATIONS ================= */
  const totalCities = cities.length;
  const totalStores = cities.reduce((sum, c) => sum + (c.totalStores || 0), 0);
  const totalUnitsSold = cities.reduce(
    (sum, c) => sum + (c.totalUnitsSold || 0),
    0
  );
  const totalInventory = cities.reduce(
    (sum, c) => sum + (c.totalInventory || 0),
    0
  );
    ...
    <div className="p-6 space-y-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow">
        <h2 className="text-xl font-bold mb-4">
          City vs Units Sold (Last 30 Days)
        </h2>
